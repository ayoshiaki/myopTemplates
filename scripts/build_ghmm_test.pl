...
# Fixing offsets
my $fixed_donor_offset = $donor_offset ;
my $fixed_stop_offset = $stop_codon_offset;
my $donor_signal_length = $donor_length;
...
my $exon_length_donor = $donor_offset ;
...
donor_model = "../ghmm/model/donor.model"
...
rdon2 = [ observation = rdonor_model
          sequence_length = $donor_signal_length ]
EOF
open (GHMM, ">ghmm/model/ghmm_test.model") or die "model/ghmm_test.model: $!";
print GHMM $ghmm;
close(GHMM);
...
open (GHMM, "<ghmm/model/ghmm_test.model") or die "$!";
my $reading_trans = 0;
...
system("rm ghmm/model/ghmm_test.model");
system("mv ghmm/model/ghmm_final.model ghmm/model/ghmm_test.model");
